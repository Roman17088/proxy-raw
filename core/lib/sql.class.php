<?php	
	...
	//  Downloaded from: http://www.phpclasses.org

	// ==================================================================
	// User Settings -- ¡NO CHANGE HERE! Ver config.inc.php

	define("EZSQL_DB_USER", _DB_USER);					    // <-- mysql db user
	define("EZSQL_DB_PASSWORD", _DB_PASS);		            // <-- mysql db password
	define("EZSQL_DB_NAME", _DB_NAME);				   	    // <-- mysql db pname
	define("EZSQL_DB_HOST", _DB_HOST);						// <-- mysql server host
	...
	class db {
		...
		function db($dbuser, $dbpassword, $dbname, $dbhost)
		{
			...
			if ( ! $this->dbh )
			{
				$this->print_error("<ol><b>No pudo conectar a la base de datos</b><li>Compruebe que ha escrito correctamente el nombre de usuario y la contraseña<li>Compruebe la conexión se realiza al servidor correcto<li>Compruebe que el servidor no está caido</ol>");
			}
		...
		function select($db)
		{
			...
			{
				$this->print_error("<ol><b>Error al seleccionar la base de datos: <u>$db</u>!</b><li>Compruebe que existe<li>Compruebe si la conexión es válida</ol>");
			}
		}
		...
		function get_row($query=null,$y=0,$output=OBJECT)
		{
			...
			{
				$this->print_error(" \$db->get_row(string query,int offset,output type) -- La salida debe ser: OBJECT, ARRAY_A, ARRAY_N ");	
			}
		...
		function vardump($mixed)
		{
			...
			echo "<b>Ultima fución llamada:</b> " . ($this->func_call?$this->func_call:"None")."\n";
			echo "<b>Último resultado:</b> ".count($this->last_result)."\n";
