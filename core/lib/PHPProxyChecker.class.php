...
class PHPProxyChecker {
        ...
        private static function sendQuery($ip,$type = '',$method='GET',$ssl=false) {
            ...
            if($type=='socks') {
				curl_setopt($handle, CURLOPT_CONNECTTIMEOUT, 11);
                curl_setopt($handle, CURLOPT_PROXYTYPE,CURLPROXY_SOCKS5);
				// Verbose, te muestra toda la información
            ...
            curl_setopt($handle, CURLOPT_REFERER, 'http://hval.es/');
			
			// Watch the connection progress, like unix curl
			...
			// Ignoro los proxies SOCKS porque con la versión de cURL que
			// tenemos no funciona bien. En un futuro próximo debemos reescribir
			// esta clase para implementar fsockopen y mejorar el rendimiento.
        ...
        private static function checkParam($typeName,$typeCode,$arServer,$ip,$arTmp,$ssl=false) {
                ...
                if($arServer['HTTP_REFERER'] == 'http://hval.es/') {
        ...
        public static function checkProxy($ip) {
            ...
            if(!empty($arServer)) {
				...
				if(isset($arServer['HTTP_X_FORWARDED_FOR'])){
